...
| GPT-5.6 intelligence | Draft a strict `EvolutionBrief` from a hash-verified, privacy-minimized neutral manifest/opportunity/event context | The current runner consumes neutral replay evidence, not automatic host evidence; no raw event IDs, paths, symbols, metadata, release data, or user/session/subject identifiers leave; no tools, permissions, approval, or activation; schema and references are revalidated locally |
| Living Studio | Display five surfaces from a labeled neutral fixture | Read-only; no host connection, observer ingestion, model call, cryptographic verification, or lifecycle action |

The intelligence transport uses the Responses API with `store: false`, fixed `gpt-5.6` selection, bounded output, and an abort timeout. `OPENAI_API_KEY` is read only when a request is sent and must never be committed or logged. Automated intelligence tests use an injected offline mock transport. Application validation proves schema and reference integrity, not that model prose is semantically true; every brief remains a human-reviewed hypothesis. A successful live GPT-5.6 run is still pending.
