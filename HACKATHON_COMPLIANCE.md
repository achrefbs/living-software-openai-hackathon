...
- Living Studio is read-only. It reads an explicitly synced synthetic snapshot and the committed sanitized GPT-5.6 proof as separate static inputs; it performs no live host or model ingestion. The proof's app, manifest, opportunity, and event-set identities do not match the current CRM or fixture, so Studio labels it independent. Rendering or relation grants no lifecycle authority.
