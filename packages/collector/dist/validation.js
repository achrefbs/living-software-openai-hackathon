import { parseWorkflowEventBatch, } from "@living-software/contracts";
...
function exactKeys(value, required, optional = []) {
    ...
    for (const key of Object.keys(value)) {
        if (!allowed.has(key))
            fail("METADATA_FIELD", `Metadata field '${key}' is not allowed`);
    }
    for (const key of required) {
        if (!(key in value))
            fail("METADATA_FIELD", `Metadata field '${key}' is required`);
    }
}
...
function validateGeometry(metadata) {
    exactKeys(metadata, [
        "targetGeometry",
        "viewport",
        "visibility",
    ...
    if (!isRecord(target))
        fail("METADATA_VALUE", "targetGeometry must be an object");
    ...
    if (!isRecord(viewport))
        fail("METADATA_VALUE", "viewport must be an object");
    ...
    if (!isRecord(visibility))
        fail("METADATA_VALUE", "visibility must be an object");
    ...
    if (!isRecord(position))
        fail("METADATA_VALUE", "position must be an object");
    exactKeys(position, ["layout", "documentX", "documentY"]);
    oneOf(position.layout, ["flow", "fixed", "sticky"], "position.layout");
    ...
    if (!isRecord(state))
        fail("METADATA_VALUE", "state must be an object");
...
export function validateObservationMetadata(candidate) {
    if (!isRecord(candidate))
        fail("METADATA_VALUE", "metadata must be an object");
    ...
    if ("interaction" in candidate) {
        ...
        oneOf(candidate.interaction, ["click", "change", "submit"], "interaction");
    ...
    if ("signal" in candidate) {
        ...
        oneOf(candidate.signal, ["dead-click", "rage-click", "correction"], "signal");
    ...
    if (keys.includes("routePhase")) {
        ...
        oneOf(candidate.routePhase, ["start", "complete"], "routePhase");
        return;
    }
    if (keys.includes("metric")) {
        exactKeys(candidate, ["metric", "value", "unit"]);
        const metric = oneOf(candidate.metric, ["lcp", "inp", "cls"], "metric");
        if (metric === "cls") {
            oneOf(candidate.unit, ["score"], "unit");
            finiteNumber(candidate.value, "value", 0, 1_000);
        }
        else {
            oneOf(candidate.unit, ["millisecond"], "unit");
    ...
    if (keys.includes("lifecycle")) {
        ...
        oneOf(candidate.lifecycle, ["pagehide"], "lifecycle");
        return;
    }
    if (keys.includes("errorCategory")) {
        exactKeys(candidate, ["errorCategory", "sanitized"]);
        oneOf(candidate.errorCategory, ["script-runtime", "promise-rejection"], "errorCategory");
...
export function resolveCollectorDefinition(definition) {
    ...
    if (!IDENTIFIER.test(app.appId))
        throw new TypeError("Invalid collector appId");
    if (!SHA256.test(app.manifestHash))
        throw new TypeError("Invalid collector manifestHash");
    if (!["development", "preview", "production"].includes(app.environment)) {
        throw new TypeError("Invalid collector environment");
    }
    if (typeof app.synthetic !== "boolean")
        throw new TypeError("Invalid collector synthetic flag");
    ...
    for (const binding of definition.eventBindings) {
        ...
        if (!IDENTIFIER.test(binding.nodeId))
            throw new TypeError("Invalid event binding nodeId");
    ...
    const limits = Object.freeze({
        maxPayloadBytes: positiveLimit(definition.limits?.maxPayloadBytes, DEFAULT_LIMITS.maxPayloadBytes, 1_000_000, "maxPayloadBytes"),
        maxEventsPerBatch: positiveLimit(definition.limits?.maxEventsPerBatch, DEFAULT_LIMITS.maxEventsPerBatch, 100, "maxEventsPerBatch"),
        maxRequestsPerMinute: positiveLimit(definition.limits?.maxRequestsPerMinute, DEFAULT_LIMITS.maxRequestsPerMinute, 10_000, "maxRequestsPerMinute"),
        maxEventsPerMinute: positiveLimit(definition.limits?.maxEventsPerMinute, DEFAULT_LIMITS.maxEventsPerMinute, 100_000, "maxEventsPerMinute"),
    });
    return { definition, bindings, limits };
}
function validateEvent(event, definition, bindings) {
    const app = definition.application;
    if (event.appId !== app.appId)
        fail("APP_MISMATCH", "Event appId does not match collector");
    ...
    if (binding === undefined)
        fail("EVENT_UNDECLARED", "Event name is not declared by the runtime map");
    if (event.kind !== binding.kind)
        fail("EVENT_KIND", "Event kind does not match its binding");
    if (event.product === undefined)
        fail("PRODUCT_REQUIRED", "Observed events require product context");
...
export function validateBatchForCollector(candidate, definition, bindings, maxEventsPerBatch) {
    let batch;
    try {
    ...
    if (sessionId === undefined)
        fail("EMPTY_BATCH", "Batch must contain events");
