...
import {
  resolveCollectorDefinition,
  validateBatchForCollector,
} from "./validation.js";

export const LEGACY_EVIDENCE_RELATIVE_PATH = ".living/data/events.ndjson" as const;
...
export function evidenceRelativePathForManifestHash(manifestHash: Sha256 | string): string {
...
function hashBatch(batch: WorkflowEventBatch) {
  return sha256(batch as unknown as JsonValue);
}

function hashRecordPayload(record: Omit<EvidenceBatchRecord, "recordHash">) {
...
interface VerifiedChainState {
  readonly records: EvidenceBatchRecord[];
  readonly batchByHash: Map<string, EvidenceBatchRecord>;
  readonly batchSequenceHashes: Map<string, string>;
  readonly eventIds: Set<string>;
  readonly sessions: Map<string, SessionState>;
}

function verifyBatchOrdering(
  record: EvidenceBatchRecord,
  state: VerifiedChainState,
): void {
  const events = record.batch.events;
  const sessionId = events[0]?.sessionId;
  if (sessionId === undefined) throw new EvidenceIntegrityError("Stored batch is empty");
  let localSequence = -1;
  ...
  candidates.forEach((candidate, index) => {
    ...
    if (resolved !== undefined) {
      validateBatchForCollector(
        ...
        resolved.definition,
        resolved.bindings,
        resolved.limits.maxEventsPerBatch,
      );
    }
...
export function parseEvidenceNdjson(
  source: string,
  definition?: CollectorDefinition,
): readonly EvidenceBatchRecord[] {
...
async function assertNotSymlink(pathname: string): Promise<void> {
...
async function ensureDirectory(pathname: string): Promise<void> {
  ...
  await mkdir(pathname, { recursive: false }).catch((error: NodeJS.ErrnoException) => {
...
async function optionalRegularFile(pathname: string): Promise<string | undefined> {
  await assertNotSymlink(pathname);
  try {
    const stat = await lstat(pathname);
...
function recordsMatchApplication(
  records: readonly EvidenceBatchRecord[],
  definition: CollectorDefinition,
): boolean {
  const app = definition.application;
  return records.every((record) =>
    record.batch.events.every((event) =>
      event.appId === app.appId &&
 ...
 */
export function parseCompatibleLegacyEvidenceNdjson(
  source: string,
  definition: CollectorDefinition,
): readonly EvidenceBatchRecord[] | undefined {
  const records = parseEvidenceNdjson(source);
  if (!recordsMatchApplication(records, definition)) return undefined;
...
export class AppendOnlyEvidenceStore {
  ...
  private readonly resolved: ReturnType<typeof resolveCollectorDefinition>;
  private readonly legacyEvidencePath: string;
  ...
  constructor(options: EvidenceStoreOptions) {
    ...
    const relative = evidenceRelativePathForManifestHash(
      this.definition.application.manifestHash,
    );
    this.evidencePath = path.join(this.rootPath, ...relative.split("/"));
    this.releaseDirectory = path.dirname(this.evidencePath);
    this.legacyEvidencePath = path.join(
      this.rootPath,
      ...LEGACY_EVIDENCE_RELATIVE_PATH.split("/"),
    );
  }
  ...
  private async readActiveUnsafe(): Promise<readonly EvidenceBatchRecord[]> {
  ...
  private async readLegacyCompatibleUnsafe(): Promise<readonly EvidenceBatchRecord[] | undefined> {
  ...
  async readVerified(): Promise<readonly EvidenceBatchRecord[]> {
    await this.serial;
    await this.prepare();
    if (await exists(this.evidencePath)) return this.readActiveUnsafe();
    return (await this.readLegacyCompatibleUnsafe()) ?? Object.freeze([]);
  }

  append(batch: WorkflowEventBatch, acceptedAt: string): Promise<AppendEvidenceResult> {
    const operation = this.serial.then(async () => {
      validateBatchForCollector(
        batch,
        this.resolved.definition,
        this.resolved.bindings,
        this.resolved.limits.maxEventsPerBatch,
      );
      await this.prepare();
      ...
      const sameSequence = records.find(
        (record) =>
          record.batch.events[0]?.sessionId === sessionId &&
          record.batch.sequence === batch.sequence,
      );
      if (sameSequence !== undefined) {
        throw new EvidenceConflictError(
          "BATCH_SEQUENCE_CONFLICT",
          "The session batch sequence already exists with different content",
        );
      }
      const seenEventIds = new Set(
        records.flatMap((record) => record.batch.events.map((event) => event.eventId)),
      );
      if (batch.events.some((event) => seenEventIds.has(event.eventId))) {
        throw new EvidenceConflictError(
          "EVENT_ID_CONFLICT",
          "One or more event ids already exist with different batch content",
        );
      }

      const sessionRecords = records.filter(
        (record) => record.batch.events[0]?.sessionId === sessionId,
      );
      ...
      if (previousSessionRecord === undefined) {
        if (batch.sequence !== 0) {
          throw new EvidenceConflictError(
            "BATCH_SEQUENCE_GAP",
            "A session's first accepted batch must have sequence zero",
          );
        }
      } else {
        if (batch.sequence !== previousSessionRecord.batch.sequence + 1) {
          throw new EvidenceConflictError(
            "BATCH_SEQUENCE_GAP",
            "Session batch sequence must increase by one",
          );
        }
        ...
        if (previousEvent !== undefined && firstEvent !== undefined) {
          if (firstEvent.sequence <= previousEvent.sequence) {
            throw new EvidenceConflictError(
              ...
              "Event sequence must increase across session batches",
            );
          }
          if (Date.parse(firstEvent.occurredAt) < Date.parse(previousEvent.occurredAt)) {
            throw new EvidenceConflictError(
              "EVENT_TIME_CONFLICT",
              "Event time must not regress across session batches",
            );
          }
        }
      }
      ...
      if (
        ...
        throw new EvidenceConflictError(
          "CLOCK_REGRESSION",
          "Evidence acceptance time must not regress",
        );
      }
      const payload = {
