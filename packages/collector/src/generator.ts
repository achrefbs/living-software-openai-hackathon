...
export function collectorDefinitionFromObservationRuntimeMap(
  runtimeMap: CollectorObservationRuntimeMap,
): CollectorDefinition {
  if (
    runtimeMap.schemaVersion !== "living.observation-runtime/v1" ||
    runtimeMap.collector.endpoint !== "/api/living/events"
  ) {
    throw new TypeError("Unsupported observation runtime map");
  }
  const candidates: CollectorEventBinding[] = [
  ...
  ].filter((binding): binding is CollectorEventBinding => binding !== undefined);
  const byName = new Map<string, CollectorEventBinding>();
  ...
  return resolveCollectorDefinition({
    ...
    eventBindings: [...byName.values()].sort((left, right) =>
...
export function generateNextCollectorFiles(
  ...
  return Object.freeze({
    route: Object.freeze({
      relativePath: "src/app/api/living/events/route.ts" as const,
      content: ROUTE_SOURCE,
    }),
    serverModule: Object.freeze({
      relativePath: "src/living-collector.generated.ts" as const,
