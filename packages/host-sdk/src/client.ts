...
export function createEventClient(options: EventClientOptions): EventClient {
  ...
  const performFlush = async (): Promise<FlushResult> => {
    ...
    while (queue.length > 0) {
      ...
      const batch: EventBatch = Object.freeze({
      ...
      try {
        ...
        if (receipt !== undefined && receipt.accepted !== pending.length) {
          throw new Error(
            `Transport accepted ${receipt.accepted} of ${pending.length} events; partial acceptance is unsupported`,
          );
        }
      } catch (error) {
  ...
  return {
    async record(candidate: unknown): Promise<RecordResult> {
      if (isClosed || isClosing) throw new EventClientClosedError();
      if (queue.length >= maxQueueSize) throw new EventQueueFullError(maxQueueSize);
    ...
    async close(): Promise<FlushResult> {
      if (isClosed) return { batches: 0, events: 0 };
      isClosing = true;
      try {
