import { identifierSchema, sha256Schema, } from "@living-software/contracts";
...
const DISALLOWED_REPLACEMENT_PATTERNS = [
    ...
    {
        label: "network API",
        pattern: /\b(?:fetch|XMLHttpRequest|WebSocket|EventSource|sendBeacon|BroadcastChannel|SharedWorker|Worker)\b/u,
    },
    { label: "process authority", pattern: /\bprocess\b/u },
    {
        label: "host module authority",
        pattern: /\b(?:child_process|fs|http|https|net|tls|worker_threads)\b/u,
    },
    ...
    },
    {
        ...
        pattern: /\b(?:secret|password|cookie|credentials?|authorization|bearer|api[_-]?key|apiKey|OPENAI_API_KEY)\b/iu,
    },
];
const DISALLOWED_CANONICAL_PATTERNS = [
    ...
    },
    {
        ...
        pattern: /\bwindow(?:\.|\[)(?:fetch|xmlhttprequest|websocket|eventsource|sendbeacon|location|open|postmessage|eval|function|localstorage|sessionstorage|indexeddb)/u,
    },
    {
    ...
    },
    {
        ...
        pattern: /\b(?:fetch|xmlhttprequest|websocket|eventsource|sendbeacon|localstorage|sessionstorage|indexeddb|broadcastchannel|sharedworker|worker|cookie)\b/u,
    },
    {
    ...
    },
    {
        ...
        pattern: /\breact\.createelement\((?:script|iframe|object|embed|form|link)\b/u,
    },
    {
        label: "form submission authority",
        pattern: /(?:<form\b|\bformaction\s*=|\bform(?:\.|\[)(?:submit|requestsubmit)|\.setattribute\(action)/u,
    },
    {
...
function invalidInput(message, cause) {
    throw new SourceEvolutionError("INVALID_INPUT", message, cause === undefined ? undefined : { cause });
}
...
function assertAllowedTarget(candidate) {
    if (candidate.includes("\\") || candidate !== candidate.replaceAll("\\", "/")) {
        throw new SourceEvolutionError("UNSAFE_TARGET", "Model patch targets must use normalized repository-relative forward slashes");
    }
    ...
    if (segments.length < 3 ||
        ...
        segments.some((segment) => segment === "" ||
            ...
            segment === ".." ||
            segment.startsWith("."))) {
        throw new SourceEvolutionError("UNSAFE_TARGET", "Model patches are limited to regular source files below src/app or src/components");
    }
    const fileName = segments.at(-1).toLowerCase();
    const targetExtension = extension(candidate);
    if (!ALLOWED_EXTENSIONS.has(targetExtension)) {
        throw new SourceEvolutionError("UNSAFE_TARGET", "Model patch targets must be .ts, .tsx, .js, .jsx, or .css files");
    }
    const lowerSegments = segments.map((segment) => segment.toLowerCase());
    if (/^route\.(?:ts|tsx|js|jsx)$/u.test(fileName) ||
        lowerSegments.some((segment) => ["api", "test", "tests", "__tests__", "e2e", "config", "configs"].includes(segment)) ||
        /(?:^|\.)(?:test|spec|config)\.(?:ts|tsx|js|jsx|css)$/u.test(fileName)) {
        throw new SourceEvolutionError("UNSAFE_TARGET", "Model patches cannot target route handlers, API surfaces, tests, or configuration files");
    }
}
function decodeEscapesForPolicy(value) {
    const decode = (digits) => {
...
function assertReplacementSafe(replacement) {
    ...
    for (const disallowed of DISALLOWED_REPLACEMENT_PATTERNS) {
        if (disallowed.pattern.test(decoded)) {
            throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", `Model patch replacement contains disallowed ${disallowed.label}`);
        }
    }
    ...
    if (/<(?:img|video|audio|source|track|iframe|script|link|object|embed)\b[^>]*\b(?:src|srcDoc|href|data)\s*=\s*\{/iu.test(decoded) ||
        /<iframe\b[^>]*\bsrc\s*=\s*["']\s*(?:https?:|\/\/)/iu.test(decoded)) {
        throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", "Model patch replacement contains a dynamic or external iframe source");
    }
}
function assertSourceCharactersSafe(value) {
    ...
    if (unsafeCharacter !== undefined) {
        ...
        throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", `Model patch postimage contains unsafe control, format, or Unicode padding character U+${codePoint.toString(16).toUpperCase().padStart(4, "0")}`);
    }
    if (EXCESSIVE_HORIZONTAL_PADDING.test(value) ||
        EXCESSIVE_VERTICAL_PADDING.test(value)) {
        throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", "Model patch postimage contains excessive whitespace padding");
    }
}
function assertScriptPostimageParses(targetPath, postimage) {
    if (extension(targetPath) === ".css")
        return;
    ...
    if (diagnostic !== undefined) {
        throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", `Model patch postimage is not syntactically valid (${diagnostic.code}): ${ts.flattenDiagnosticMessageText(diagnostic.messageText, " ")}`);
    }
}
function assertExecutablePostimage(targetPath, postimage) {
    assertSourceCharactersSafe(postimage);
    assertScriptPostimageParses(targetPath, postimage);
}
function locateEdits(preimage, edits) {
    ...
    const located = edits.map((edit) => {
        ...
        if (start < 0 || start !== preimage.lastIndexOf(edit.anchor)) {
            throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", "Every model patch anchor must occur exactly once in the target preimage");
        }
        return {
            ...edit,
            start,
    ...
    const ordered = [...located].sort((left, right) => left.start - right.start || left.end - right.end);
...
function applyLocatedEdits(preimage, edits) {
...
function compileModelPatchInternal(proposalInput, preimage, validateExecutableSource) {
    ...
    if (preimageHash !== proposal.target.preimageHash) {
        throw new SourceEvolutionError("TARGET_PREIMAGE_MISMATCH", "Model patch target preimage hash does not match the exact supplied bytes");
    }
    const edits = locateEdits(preimage, proposal.edits);
    const removedBytes = edits.reduce((total, edit) => total + byteLength(edit.anchor), 0);
    const addedBytes = edits.reduce((total, edit) => total + byteLength(edit.replacement), 0);
    const changedLines = edits.reduce((total, edit) => total + lineCount(edit.anchor) + lineCount(edit.replacement), 0);
    if (removedBytes + addedBytes > MAX_DIFF_BYTES ||
        changedLines > MAX_DIFF_LINES) {
        throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", "Model patch exceeds the bounded diff budget");
    }
    ...
    if (postimage === preimage ||
        ...
        throw new SourceEvolutionError("UNSUPPORTED_ADAPTER_INPUT", "Model patch must produce one nonempty changed source file within the size limit");
    }
...
export function compileModelPatch(proposalInput, preimage) {
    return compileModelPatchInternal(proposalInput, preimage, true);
}
export function compileStoredModelPatchForIntegrity(proposalInput, preimage) {
    return compileModelPatchInternal(proposalInput, preimage, false);
}
//# sourceMappingURL=model-patch.js.map