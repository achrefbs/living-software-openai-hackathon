...
const LEADS_SELECTOR =
  "  const leads = useCrm((state) => state.leads);";
...
function lineEnding(source: string): "\n" | "\r\n" {
  if (/\r(?!\n)/u.test(source)) {
    throw new SourceEvolutionError(
      "UNSUPPORTED_ADAPTER_INPUT",
      "The target contains unsupported carriage-return line endings",
    );
  }
  ...
  if (hasCrLf && withoutCrLf.includes("\n")) {
    throw new SourceEvolutionError(
      "UNSUPPORTED_ADAPTER_INPUT",
      "The target contains mixed line endings",
    );
  }
  return hasCrLf ? "\r\n" : "\n";
}

function withLineEnding(fragment: string, newline: "\n" | "\r\n"): string {
  return newline === "\n" ? fragment : fragment.replaceAll("\n", "\r\n");
}

export function compileLeadReviewNavigation(preimage: string): string {
  if (preimage.length === 0 || preimage.length > 2_000_000) {
    throw new SourceEvolutionError(
      "UNSUPPORTED_ADAPTER_INPUT",
      "The target preimage must contain between 1 byte and 2 MB of text",
    );
  }
  for (const hook of SOURCE_EVOLUTION_HOOKS) {
    if (preimage.includes(`data-testid="${hook}"`)) {
      throw new SourceEvolutionError(
        ...
        `The target already contains the '${hook}' evolution hook`,
      );
    }
  }
  ...
  let postimage = replaceExactlyOnce(
    preimage,
    paramsAnchor,
    `${paramsAnchor}${newline}${withLineEnding(LEADS_SELECTOR, newline)}`,
    "lead parameter",
  );
  postimage = replaceExactlyOnce(
    postimage,
    nameAnchor,
    withLineEnding(POSITION_DERIVATION, newline),
    "lead display name",
  );
  postimage = replaceExactlyOnce(
    postimage,
    navigationAnchor,
    withLineEnding(NAVIGATION_MARKUP, newline),
    "lead header",
  );
...
export type StaticProofCheck = Readonly<{
  id: (typeof SOURCE_EVOLUTION_TESTS)[number];
  status: "passed";
  detail: string;
}>;

export function verifyLeadReviewNavigation(
  preimage: string,
  postimage: string,
): readonly StaticProofCheck[] {
  const expected = (() => {
    const newline = lineEnding(preimage);
    const paramsAnchor = withLineEnding(PARAMS_ANCHOR, newline);
    let value = replaceExactlyOnce(
      preimage,
      paramsAnchor,
      `${paramsAnchor}${newline}${withLineEnding(LEADS_SELECTOR, newline)}`,
      "lead parameter",
    ...
    value = replaceExactlyOnce(
      value,
      withLineEnding(NAME_ANCHOR, newline),
      withLineEnding(POSITION_DERIVATION, newline),
      "lead display name",
    );
    return replaceExactlyOnce(
      value,
      withLineEnding(NAVIGATION_ANCHOR, newline),
      withLineEnding(NAVIGATION_MARKUP, newline),
      "lead header",
    );
  })();
  ...
  if (postimage !== expected || postimage === preimage) {
    throw new SourceEvolutionError(
      "UNSUPPORTED_ADAPTER_INPUT",
      "The postimage is not the exact deterministic adapter output",
    );
  }
  for (const hook of SOURCE_EVOLUTION_HOOKS) {
    if (count(postimage, `data-testid="${hook}"`) !== 1) {
      throw new SourceEvolutionError(
        "UNSUPPORTED_ADAPTER_INPUT",
        `The postimage must contain exactly one '${hook}' hook`,
      );
    }
  }
  ...
  if (prohibitedTokens.some((token) => insertedPolicySurface.includes(token))) {
    throw new SourceEvolutionError(
      "UNSUPPORTED_ADAPTER_INPUT",
      "The deterministic patch contains a prohibited authority token",
    );
  }
  if (
    !postimage.includes("const leads = useCrm((state) => state.leads);") ||
    ...
    throw new SourceEvolutionError(
      "UNSUPPORTED_ADAPTER_INPUT",
      "Navigation must derive only from the existing host lead collection",
    );
  }

  const details: Record<(typeof SOURCE_EVOLUTION_TESTS)[number], string> = {
