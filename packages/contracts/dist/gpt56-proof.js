...
export const GPT56_PROOF_SCHEMA_VERSION = "living.gpt56-proof/v2";
...
const sourceCommitSchema = z
    .string()
    .regex(/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/, "Expected a lowercase 40- or 64-character source commit");
...
function expectedClaimScope(origin) {
    switch (origin) {
        case "synthetic":
...
export const gpt56EvolutionBriefSchema = z
    ...
    const metricNames = brief.evidenceCitations.metrics.map((metric) => metric.name);
    ...
    if (brief.evidenceScope.claimScope !==
...
export const gpt56ProofSchema = z
    ...
    if (proof.request.transportRequestedModel !==
    ...
    if (proof.selectedProvider === "codex") {
        if (proof.request.transportRequestedModel !== "gpt-5.6-terra" ||
    ...
    else if (proof.request.transportRequestedModel !== "gpt-5.6" ||
    ...
    const aliasedEventIds = new Set(provenance.evidenceAliases.map((entry) => entry.eventId));
    for (const [index, eventId] of draft.evidenceCitations.sampleEventIds.entries()) {
    ...
    if (provenance.evidenceAliases.length > proof.evidence.eventCount ||
    ...
    if (proof.evidence.sessionCount > proof.evidence.eventCount ||
...
export function parseGpt56Proof(input) {
    return gpt56ProofSchema.parse(input);
}
//# sourceMappingURL=gpt56-proof.js.map