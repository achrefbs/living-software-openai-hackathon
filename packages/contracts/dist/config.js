import { z } from "zod";
import { closedObjectJsonSchema, eventNameSchema, identifierSchema, jsonObjectSchema, relativePathSchema, } from "./primitives.js";
...
export const eventDefinitionSchema = z
    ...
    .strict();
function hasSafeCollectorPath(candidate) {
...
function isCollectorEndpoint(candidate) {
    if (candidate.trim() !== candidate || /\s/.test(candidate))
        return false;
    if (candidate.startsWith("/"))
        return hasSafeCollectorPath(candidate);
...
export const livingConfigSchema = z
    ...
    if (config.privacy.identifierMode === "pseudonymous" &&
    ...
    const adapterKeys = config.adapters.map((adapter) => `${adapter.id}@${adapter.version}`);
...
export function parseLivingConfig(input) {
