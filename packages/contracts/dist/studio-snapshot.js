...
export const STUDIO_SNAPSHOT_SCHEMA_VERSION = "living.studio-snapshot/v1";
...
export const studioSnapshotSchema = z
    ...
    if (metricReport.appId !== application.appId ||
    ...
    if (snapshot.evidence.events !== projectedEventCount ||
    ...
    const variantJourneys = new Set();
    const assignedCases = new Set();
    for (const [index, variant] of snapshot.workflows.variants.entries()) {
        ...
        if (outcomes.succeeded !== variant.outcomes.succeeded ||
        ...
        if (resolvedCases === variant.caseIds.length &&
    ...
    if (opportunity !== undefined) {
        if (opportunity.appId !== application.appId ||
        ...
        if (opportunity.evidence.subjectCount > snapshot.workflows.cases.length ||
        ...
        if (Date.parse(opportunity.window.from) < Date.parse(metricReport.window.from) ||
...
export function parseStudioSnapshot(input) {
    return studioSnapshotSchema.parse(input);
}
//# sourceMappingURL=studio-snapshot.js.map