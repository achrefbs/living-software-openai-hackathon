...
export const LIVE_EVENT_SCHEMA_VERSION = "living.live-event/v1" as const;
export const LIVE_REPLAY_SCHEMA_VERSION = "living.live-replay/v1" as const;
export const LIVE_STATE_SCHEMA_VERSION = "living.live-state/v1" as const;
export const LIVE_COMMAND_SCHEMA_VERSION = "living.live-command/v1" as const;
export const LIVE_COMMAND_RESULT_SCHEMA_VERSION =
  "living.live-command-result/v1" as const;
export const DETECTOR_PROGRESS_SCHEMA_VERSION =
  "living.detector-progress/v1" as const;
...
const safeSummarySchema = z
  ...
  .refine(
    (value) => !/[\u0000-\u001f\u007f]/u.test(value),
    "Live summaries cannot contain control characters",
  );
...
export const detectorProgressSchema = z
  ...
  .superRefine((progress, context) => {
    ...
    if (
      isRepeatedSequence !== (progress.affectedSessions !== undefined) ||
      isRepeatedSequence !==
      ...
      context.addIssue({
        ...
        message:
          "Only repeated-sequence progress carries affected and minimum independent-session counts",
      });
    }
    const thresholdExpected =
      progress.affectedCases >= progress.minimumAffectedCases &&
      (!isRepeatedSequence ||
    ...
    if (progress.thresholdMet !== thresholdExpected) {
      context.addIssue({
        ...
        path: ["thresholdMet"],
        message:
          "Threshold state must match the required affected-case and independent-session counts",
      });
    }
    if (
      (progress.signalKind === "backtracking") !==
      ...
      context.addIssue({
        ...
        message:
          "Only backtracking progress carries a revisits-per-case threshold",
      });
    }
  });

export type DetectorProgress = z.infer<typeof detectorProgressSchema>;

export function parseDetectorProgress(input: unknown): DetectorProgress {
...
const allowedStatusActors: Readonly<
  Record<z.infer<typeof liveEventStageSchema>, readonly string[]>
> = {
  connection: ["system"],
  mapping: ["system"],
  installation: ["system"],
...
export const liveEventSchema = z
  ...
  .superRefine((event, context) => {
    ...
    if (
      (event.kind === "evidence" || event.kind === "detector-progress") &&
    ...
    if (event.kind === "detector-progress") {
      ...
      const hasOpportunity =
        event.refs.opportunityId !== undefined &&
      ...
      if (hasOpportunity !== event.facts.progress.thresholdMet) {
        context.addIssue({
          ...
          path: ["refs"],
          message:
            "Only threshold-complete detector events may carry exact Opportunity identity",
        });
      }
    }
    if (event.kind === "model") {
    ...
    if (event.kind === "proof-check") {
      if (
        (event.facts.status === "passed" && event.state !== "completed") ||
    ...
    if (event.kind === "source-transition") {
      const expectedStage =
        event.facts.transition === "apply" ? "application" : "rollback";
      const expectedFrom =
        event.facts.transition === "apply"
      ...
      if (
        ...
        context.addIssue({
          ...
          path: ["refs"],
          message:
            "Source transitions require an exact sealed evolution, receipt, path, and hash direction",
        });
      }
    }
    if (
...
export const liveReplaySchema = z
  ...
  .superRefine((replay, context) => {
    ...
    let expectedSequence =
      replay.afterSequence === null ? 0 : replay.afterSequence + 1;
    let previousEvent: LiveEvent | undefined;
...
export function parseLiveReplay(input: unknown): LiveReplay {
...
export const liveStateSchema = z
  ...
  .superRefine((state, context) => {
    ...
    const detectorIds = state.detectorProgress.map(
      (progress) => progress.detectorId,
    );
    ...
    if (
      state.evidence.acceptedBatchCount === 0 &&
      ...
      context.addIssue({
        ...
        path: ["evidence"],
        message:
          "An empty evidence state cannot claim events, cases, sessions, or a chain head",
      });
    }
    if (
      state.evidence.acceptedBatchCount > 0 &&
    ...
    if (
      (state.runtime.status === "not-available") !==
...
export function parseLiveState(input: unknown): LiveState {
...
export type LiveCommandEnvelope = z.infer<typeof liveCommandEnvelopeSchema>;

export function parseLiveCommandEnvelope(input: unknown): LiveCommandEnvelope {
...
export function parseLiveCommandResult(input: unknown): LiveCommandResult {
