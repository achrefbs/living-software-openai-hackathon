...
import type { HostInterfaceDescriptor } from "./product-manifest.js";
import {
  contentRefSchema,
  identifierSchema,
  invariantResult,
...
export const capabilityContractSchema = z
  ...
  .superRefine((contract, context) => {
    ...
    const grantKeys = contract.grants.map(
      (grant) => `${grant.operationId}@${grant.operationVersion}`,
    );
    ...
    const totalGrantedCalls = contract.grants.reduce(
      (total, grant) => total + grant.maxCalls,
      0,
    );
...
export type TemplateValue =
  ...
  | {
      $value:
        ...
        | {
            source: "context";
            key: "actorId" | "sessionId" | "subjectType" | "subjectId";
          }
...
export function validateCapabilityArtifactAgainstContract(
  artifact: CapabilityArtifact,
  contract: CapabilityContract,
  host: HostInterfaceDescriptor,
): InvariantResult {
  const issues: string[] = [];
  ...
  if (
    artifact.target.manifestHash !== contract.target.manifestHash ||
  ...
  if (
    !host.extensionPoints.some(
      (point) => point.id === artifact.target.extensionPointId,
    )
  ) {
  ...
  const usedCalls = new Map<string, number>();
...
function collectStepReferences(value: TemplateValue): string[] {
