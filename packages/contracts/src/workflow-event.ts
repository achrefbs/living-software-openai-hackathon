...
import type { LivingConfig } from "./config.js";
...
export const workflowEventSchema = z
  ...
  .superRefine((event, context) => {
    if (
      (event.provenance.source === "simulator" ||
...
export function validateWorkflowEventAgainstConfig(
  event: WorkflowEvent,
  config: LivingConfig,
): InvariantResult {
  const issues: string[] = [];
  ...
  if (
    config.privacy.identifierMode === "anonymous" &&
