...
export const metricReportSchema = z
  ...
  .superRefine((report, context) => {
    ...
    const keys = report.values.map(
      (metric) =>
        `${metric.id}|${metric.productNodeId ?? ""}|${metric.routeNodeId ?? ""}|${metric.viewportClass ?? ""}`,
    );
...
export function parseMetricReport(input: unknown): MetricReport {
