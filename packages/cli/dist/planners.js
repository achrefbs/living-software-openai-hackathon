import { hostInterfaceDescriptorSchema, parseLivingConfig, parseProductManifest, } from "@living-software/contracts";
import { canonicalJson, sha256 } from "./canonical.js";
import { parseNextJsHostFixture } from "./fixture.js";
import { CLI_PLAN_SCHEMA_VERSION, } from "./types.js";
...
function buildHostInterface(fixture) {
    ...
    if (extensionPoints.length === 0 && operations.length === 0)
        return undefined;
    const content = {
...
export function buildLivingConfig(fixtureCandidate) {
...
export function buildProductManifest(fixtureCandidate) {
    const fixture = parseNextJsHostFixture(fixtureCandidate);
...
function fileChange(action, path, value, reason) {
...
export function planInit(fixtureCandidate) {
    const fixture = parseNextJsHostFixture(fixtureCandidate);
    ...
    if (hostInterface !== undefined) {
        changes.push(fileChange("create", HOST_INTERFACE_PATH, hostInterface, "Declare bounded host operations and extension points."));
    }
    return {
...
export function planMap(fixtureCandidate) {
    const fixture = parseNextJsHostFixture(fixtureCandidate);
    const manifest = buildProductManifest(fixture);
    return {
        schemaVersion: CLI_PLAN_SCHEMA_VERSION,
...
export function planDoctor(fixtureCandidate, inputs = {}) {
    const fixture = parseNextJsHostFixture(fixtureCandidate);
...
export function planUninstall(fixtureCandidate) {
    const fixture = parseNextJsHostFixture(fixtureCandidate);
...
export function planCommand(command, fixture, doctorInputs = {}) {
    switch (command) {
        case "init":
