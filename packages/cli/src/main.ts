...
export function formatAnalyzeResult(output: Record<string, unknown>): string {
  ...
  lines.push(
    `Matrix: ${metricValues.length} privacy-safe behavior measurements are ready for AI discovery.`,
    "Analysis: analyze builds the behavior matrix; it does not choose or gate a feature.",
    "Clues: route/action/outcome sequences, frequency, timing, failures, visibility, target size, distance, scrolling, LCP, INP, and CLS.",
    "AI decision: during improve, GPT-5.6 examines the full matrix, chooses the pattern that matters, and proposes the software change.",
  );
