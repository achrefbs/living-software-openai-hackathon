...
export type SourceCandidateBrief = Readonly<{
...
export type SourceCandidateErrorCode =
  | "INVALID_BRIEF"
  | "AFFECTED_NODE_MISSING"
  | "NO_ELIGIBLE_SOURCE"
  | "UNSAFE_PATH"
  | "SOURCE_MISSING"
  | "SYMLINK_REJECTED"
...
export class SourceCandidateError extends Error {
  public constructor(
    public readonly code: SourceCandidateErrorCode,
    message: string,
  ) {
    super(message);
...
export type CollectSourceCandidatesInput = Readonly<{
  repositoryRoot: string;
  manifest: ProductManifest;
  brief: SourceCandidateBrief;
}>;
...
function normalizeSourcePath(candidate: string): string {
  ...
  if (
    ...
    throw new SourceCandidateError(
      "UNSAFE_PATH",
      `Source candidate must be a canonical repository-relative path: ${candidate}`,
    );
  }
  return normalized;
}

function isEligibleSourcePath(relative: string): boolean {
  ...
  return (
    ...
    !segments
      ...
      .some(
        (segment) =>
          segment.startsWith(".") || EXCLUDED_SEGMENT.test(segment),
      ) &&
...
function validateBrief(brief: SourceCandidateBrief): readonly string[] {
  if (
    !Array.isArray(brief.affectedProductNodeIds) ||
    brief.affectedProductNodeIds.length < 1 ||
    brief.affectedProductNodeIds.length > 32 ||
    brief.affectedProductNodeIds.some(
      (nodeId) =>
    ...
    throw new SourceCandidateError(
      "INVALID_BRIEF",
      "Source selection requires 1 to 32 valid affected product node ids",
    );
  }
...
function fileState(stat: BigIntStats): FileState {
  return {
    size: stat.size,
    device: stat.dev,
    inode: stat.ino,
...
function isSameFileState(left: FileState, right: FileState): boolean {
  return (
...
async function assertPathHasNoSymlink(
  root: string,
  relative: string,
): Promise<Readonly<{ absolute: string } & FileState>> {
  const segments = relative.split("/");
  let current = root;
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      if (isMissing(error)) {
        throw new SourceCandidateError(
          "SOURCE_MISSING",
          `Mapped source file no longer exists: ${relative}`,
        );
      }
      throw error;
    }
    if (stat.isSymbolicLink()) {
      throw new SourceCandidateError(
        "SYMLINK_REJECTED",
        `Source candidate traverses a symbolic link: ${relative}`,
      );
    }
    ...
    if ((!isLast && !stat.isDirectory()) || (isLast && !stat.isFile())) {
      throw new SourceCandidateError(
        "UNSAFE_FILE",
        `Source candidate is not a regular file below regular directories: ${relative}`,
      );
    }
    if (isLast) {
      return {
...
function contentHash(content: Buffer): Sha256 {
  return `sha256:${createHash("sha256").update(content).digest("hex")}`;
}

function containsBinaryControlByte(content: Buffer): boolean {
  return content.some(
    (byte) =>
...
function candidateOpenFlags(relative: string): number {
  ...
  if (typeof fsConstants.O_NOFOLLOW !== "number") {
    throw new SourceCandidateError(
      "UNSAFE_FILE",
      `The runtime cannot safely open source candidates without following links: ${relative}`,
    );
  }
...
async function openCandidate(
  ...
  try {
    ...
    if (code === "ELOOP" || code === "EMLINK") {
      throw new SourceCandidateError(
        "SYMLINK_REJECTED",
        `Source candidate became a symbolic link while it was being opened: ${relative}`,
      );
    }
    if (code === "ENOENT" || code === "ENOTDIR") {
      throw new SourceCandidateError(
        "SOURCE_CHANGED",
        `Source candidate changed while it was being opened: ${relative}`,
      );
    }
    if (
      code === "EINVAL" ||
      ...
      throw new SourceCandidateError(
        "UNSAFE_FILE",
        `The filesystem cannot safely open source candidates without following links: ${relative}`,
      );
    }
    throw new SourceCandidateError(
      "UNSAFE_FILE",
      `Source candidate could not be safely opened: ${relative}`,
    );
  }
}
...
async function readCandidate(root: string, relative: string): Promise<Readonly<{
  ...
  if (!isInside(root, before.absolute)) {
    throw new SourceCandidateError(
      "UNSAFE_PATH",
      `Source candidate escaped the repository root: ${relative}`,
    );
  }
  if (before.size > BigInt(SOURCE_CANDIDATE_LIMITS.maxFileBytes)) {
    throw new SourceCandidateError(
      "FILE_TOO_LARGE",
      `Source candidate exceeds ${SOURCE_CANDIDATE_LIMITS.maxFileBytes} bytes: ${relative}`,
    );
  }
  ...
  try {
    ...
    if (!handleBeforeStat.isFile()) {
      throw new SourceCandidateError(
        "UNSAFE_FILE",
        `Source candidate is not an opened regular file: ${relative}`,
      );
    }
    ...
    if (
      ...
      throw new SourceCandidateError(
        "SOURCE_CHANGED",
        `Source candidate changed while it was being read: ${relative}`,
      );
    }
  } finally {
    await handle.close();
  ...
  if (containsBinaryControlByte(bytes)) {
    throw new SourceCandidateError(
      "BINARY_CONTENT",
      `Source candidate contains binary control bytes: ${relative}`,
    );
  }
  ...
  try {
    ...
    throw new SourceCandidateError(
      "INVALID_UTF8",
      `Source candidate is not valid UTF-8: ${relative}`,
    );
  }
  return {
...
export async function collectSourceCandidates(
  input: CollectSourceCandidatesInput,
  ...
  for (const nodeId of affectedNodeIds) {
    ...
    if (node === undefined) {
      throw new SourceCandidateError(
        "AFFECTED_NODE_MISSING",
        `Affected product node is absent from the manifest: ${nodeId}`,
      );
    }
    for (const source of node.provenance.sources) {
      const relative = normalizeSourcePath(source.path);
      if (isEligibleSourcePath(relative)) sourcePaths.add(relative);
    }
  }
  ...
  if (sourcePaths.size === 0) {
    throw new SourceCandidateError(
      "NO_ELIGIBLE_SOURCE",
      "Affected product nodes do not reference an eligible UI source file",
    );
  }
  ...
  if (!rootStat.isDirectory()) {
    throw new SourceCandidateError(
      "UNSAFE_PATH",
      "Source candidate repository root must be a directory",
    );
  }
  ...
  for (const relative of [...sourcePaths].sort((left, right) => left.localeCompare(right))) {
    if (selected.length >= SOURCE_CANDIDATE_LIMITS.maxFiles) break;
  ...
  if (selected.length === 0) {
    throw new SourceCandidateError(
      "NO_ELIGIBLE_SOURCE",
      "Eligible source files could not fit inside the source projection budget",
    );
  }
