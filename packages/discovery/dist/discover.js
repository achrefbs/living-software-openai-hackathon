...
import { parseLivingConfig, parseProductManifest, } from "@living-software/contracts";
import { analyzeSource, } from "./ast.js";
import { DEFAULT_LIMITS, securelyReadSourceTree, } from "./security.js";
...
function stableNodeId(kind, semanticKey) {
...
function metricId(target, eventName) {
...
function parseNextVersion(range) {
    const match = range.match(/(?:^|[^0-9])(\d+)\.(\d+)(?:\.(\d+))?/u);
    if (match === null) {
        throw new DiscoveryError("next-version-unreadable", `Could not determine a concrete Next.js version from ${JSON.stringify(range)}`);
    }
    const major = Number(match[1]);
    const minor = Number(match[2]);
    const patch = Number(match[3] ?? "0");
    if (major < 15 || (major === 15 && minor < 3)) {
        throw new DiscoveryError("unsupported-next-version", `Next.js ${major}.${minor}.${patch} is unsupported; discovery requires 15.3.0 or newer`);
    }
...
function parsePackage(file) {
    ...
    if (nextRange === undefined) {
        throw new DiscoveryError("next-dependency-missing", "package.json does not declare Next.js");
    }
...
function routeParts(relativePath) {
    const match = relativePath.match(/^(?:src\/)?app\/(.*\/)?(page|layout|route)\.(?:[cm]?js|jsx|ts|tsx)$/iu);
    if (match === null)
        return undefined;
...
function sourceReference(relativePath, revision, line, symbol) {
    return {
        path: relativePath,
        revision,
        ...(line === undefined ? {} : { line }),
...
function routeMatches(candidate, declared) {
    const candidateParts = normalizedPath(candidate).split("/");
    const declaredParts = normalizedPath(declared).split("/");
    if (candidateParts.length !== declaredParts.length)
        return false;
    return declaredParts.every((part, index) => part.startsWith(":") ||
        part.startsWith("*") ||
        part === candidateParts[index]);
}
function resolveImport(currentPath, specifier, knownPaths) {
    let base;
...
function cssSelector(locator) {
    const escaped = locator.normalizedValue
        ...
        .replace(/"/gu, '\\"');
    if (!locator.dynamic)
        return `[${locator.attribute}="${escaped}"]`;
    const [prefix = "", suffix = ""] = escaped.split("{*}", 2);
    if (prefix !== "" && suffix !== "") {
        return `[${locator.attribute}^="${prefix}"][${locator.attribute}$="${suffix}"]`;
    }
    if (prefix !== "")
        return `[${locator.attribute}^="${prefix}"]`;
    if (suffix !== "")
        return `[${locator.attribute}$="${suffix}"]`;
    return `[${locator.attribute}]`;
}
function locatorMatch(locator) {
    if (!locator.dynamic)
        return { kind: "exact", value: locator.normalizedValue };
...
function isGenericCatchAllLocator(locator) {
    return (locator.dynamic &&
        locator.normalizedValue.trim().replaceAll("{*}", "*") === "*");
}
function boundEvents(captures) {
    return captures.map((capture) => capture === "geometry"
        ? "layout.geometry"
        : capture === "view"
            ? "navigation.view"
            : `action.${capture}`);
}
function isStrongEntity(entity, relativePath) {
    if (/(?:Props|Options|Config|Result|Response|Parameters)$/u.test(entity.name)) {
        return false;
    }
    const domainPath = /(?:^|\/)(?:domain|entities|models|types)(?:\/|\.)/iu.test(relativePath);
    return (entity.fields.length >= (domainPath ? 2 : 4) ||
...
function buildConfig(appId, displayName, collectorEndpoint) {
...
export async function discoverNextApp(options) {
    const limits = {
        ...DEFAULT_LIMITS,
        ...options.limits,
    };
    ...
    const analysesByPath = new Map(analyses.map((entry) => [entry.analysis.path, entry.analysis]));
    const knownPaths = new Set(analysesByPath.keys());
    const routeFiles = [];
    for (const entry of analyses) {
        const route = routeParts(entry.file.relativePath);
        if (route !== undefined)
            routeFiles.push({ ...entry, ...route });
    }
    const pages = routeFiles.filter((route) => route.kind === "page");
    if (pages.length === 0) {
        throw new DiscoveryError("app-router-pages-missing", "No Next.js App Router page files were discovered under app/ or src/app/");
    }
    const nodes = new Map();
    const edges = new Map();
    const runtimeLocators = [];
    const metrics = [];
    const diagnostics = [];
    const fileSurfaces = new Map();
    ...
    function addNode(node) {
        ...
        const sources = [...current.provenance.sources, ...node.provenance.sources]
            .filter((source, index, all) => all.findIndex((candidate) => candidate.path === source.path &&
            candidate.line === source.line &&
            candidate.symbol === source.symbol) === index)
            .sort((left, right) => `${left.path}:${left.line ?? 0}:${left.symbol ?? ""}`.localeCompare(`${right.path}:${right.line ?? 0}:${right.symbol ?? ""}`));
        nodes.set(node.id, {
            ...current,
    ...
    function addEdge(edge) {
        ...
        const sources = [...current.provenance.sources, ...edge.provenance.sources]
            .filter((source, index, all) => all.findIndex((candidate) => candidate.path === source.path && candidate.line === source.line) === index)
    ...
    function provenance(origin, confidence, source) {
    ...
    for (const page of pages) {
        ...
        runtimeLocators.push({
            ...
            match: page.route.includes(":") || page.route.includes("*")
    ...
    for (const { analysis } of analyses) {
        ...
        const declaredComponents = routeParts(analysis.path) === undefined ? analysis.components : [];
        for (const component of declaredComponents) {
            ...
            addNode({
                ...
                provenance: provenance("scanned", 1, sourceReference(analysis.path, revision, component.line, component.name)),
                attributes: { surfaceType: "component", export: component.name },
            });
            surfaces.push(componentId);
        }
        if (surfaces.length > 0)
            fileSurfaces.set(analysis.path, [...new Set(surfaces)]);
        for (const entity of analysis.entities.filter((candidate) => isStrongEntity(candidate, analysis.path))) {
            ...
            addNode({
                ...
                provenance: provenance("scanned", 0.96, sourceReference(analysis.path, revision, entity.line, entity.name)),
                attributes: {
    ...
    for (const { analysis } of analyses) {
        ...
        for (const imported of analysis.imports) {
            ...
            if (analysis.usedJsxNames.has(imported.localName)) {
                const targetId = imported.importedName === "default"
                ...
                if (targetId !== undefined) {
                    for (const surfaceId of currentSurfaces) {
                        ...
                        addEdge({
                            ...
                            to: targetId,
                            relation: "renders",
                            provenance: provenance("inferred", 0.94, sourceReference(analysis.path, revision, imported.line, imported.localName)),
                        });
                    }
                }
            }
            ...
            if (entityId !== undefined) {
                for (const surfaceId of currentSurfaces) {
                    addEdge({
                        ...
                        to: entityId,
                        relation: "reads",
                        provenance: provenance("inferred", imported.typeOnly ? 0.9 : 0.72, sourceReference(analysis.path, revision, imported.line, imported.localName)),
                    });
                }
            }
        }
        for (const link of analysis.links) {
            const destination = [...routeIds.entries()].find(([route]) => routeMatches(normalizedPath(link.target), route));
            ...
            for (const surfaceId of currentSurfaces) {
                addEdge({
                    ...
                    provenance: provenance("scanned", 0.99, sourceReference(analysis.path, revision, link.line)),
                });
            }
        }
        for (const call of analysis.fetches) {
            const endpoint = [...endpointIds.entries()].find(([route]) => routeMatches(normalizedPath(call.target), route));
            ...
            for (const surfaceId of currentSurfaces) {
                addEdge({
                    ...
                    to: endpoint[1],
                    relation: "calls",
                    provenance: provenance("scanned", 0.96, sourceReference(analysis.path, revision, call.line)),
                });
            }
        }
        const locatorOccurrences = new Map();
        for (const locator of analysis.locators) {
            ...
            const interactive = locator.captures.some((capture) => ["activate", "change", "submit"].includes(capture));
            if (interactive && isGenericCatchAllLocator(locator)) {
                diagnostics.push({
                    severity: "warning",
                    code: "generic-action-locator-skipped",
                    message: `Generic interactive ${locator.attribute} expression at line ${locator.line} was skipped because it has no stable action-family prefix or suffix`,
                    path: analysis.path,
                });
                continue;
            }
            ...
            const source = sourceReference(analysis.path, revision, locator.line, locator.symbol);
            addNode({
                id: locatorNodeId,
            ...
            const owner = (locator.symbol === undefined
                ? undefined
            ...
            for (const capture of locator.captures) {
                const eventName = capture === "geometry"
                    ? "layout.geometry"
                    : capture === "view"
                        ? "navigation.view"
                        : `action.${capture}`;
        ...
        for (const storage of analysis.storage) {
            const integrationId = stableNodeId("integration", `local-storage:${storage.key}`);
    ...
    const sortedEdges = [...edges.values()].sort((left, right) => `${left.from}\0${left.to}\0${left.relation}`.localeCompare(`${right.from}\0${right.to}\0${right.relation}`));
    ...
    const canReusePriorTimestamp = options.previous?.sourceDigest === revision &&
    ...
    const manifest = parseProductManifest({
        ...semanticManifest,
        generatedAt,
        contentHash: canReusePriorTimestamp && options.previous !== undefined
            ? options.previous.manifest.contentHash
            : contentHash,
    });
    const config = buildConfig(appId, displayName, options.collectorEndpoint ?? "http://127.0.0.1:4318/v1/living/events");
    const uniqueLocators = runtimeLocators
        .filter((locator, index, all) => all.findIndex((candidate) => candidate.nodeId === locator.nodeId &&
        candidate.strategy === locator.strategy &&
        candidate.selector === locator.selector) === index)
        .sort((left, right) => `${left.nodeId}\0${left.selector}`.localeCompare(`${right.nodeId}\0${right.selector}`));
    ...
    return {
        ...
        diagnostics: [
            ...scan.diagnostics,
            ...diagnostics.sort((left, right) => `${left.path ?? ""}:${left.message}`.localeCompare(`${right.path ?? ""}:${right.message}`)),
            {
                severity: "info",
                code: "static-analysis-only",
                message: "Discovery parsed source text without importing host modules or executing configuration and scripts",
            },
            {
                severity: "info",
                code: "runtime-geometry-pending",
                message: "Geometry metrics are prepared from stable locators; coordinates require the runtime capture adapter",
            },
        ],
        stats: {
