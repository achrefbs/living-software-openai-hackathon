...
function isApplicationSourceFile(relativePath) {
    const base = path.posix.basename(relativePath).toLowerCase();
    if (base === "package.json" && relativePath === "package.json")
        return true;
    if (!SOURCE_EXTENSIONS.has(path.posix.extname(base)))
        return false;
    const segments = relativePath.toLowerCase().split("/");
    if (segments.some((segment) => COLOCATED_HARNESS_DIRECTORIES.has(segment))) {
        return false;
    }
    if (HARNESS_FILE.test(base))
        return false;
    return APPLICATION_SOURCE_PREFIXES.some((prefix) => relativePath.toLowerCase().startsWith(prefix));
}
...
export async function securelyReadSourceTree(repositoryRoot, limits) {
    ...
    if (rootStat === undefined || !rootStat.isDirectory()) {
        throw new DiscoveryError("invalid-root", `Discovery root is not a directory: ${requestedRoot}`);
    }
    if (rootStat.isSymbolicLink()) {
        throw new DiscoveryError("symlink-root-rejected", "Discovery refuses a repository root that is a symbolic link");
    }
    ...
    async function walk(directory) {
        if (!isWithin(root, directory)) {
            throw new DiscoveryError("path-escape-rejected", `Discovery path escaped the repository root: ${directory}`);
        }
        ...
        for (const entry of entries) {
            ...
            if (GENERATED_INTEGRATION_FILES.has(relativePath))
                continue;
            ...
            if (entry.isDirectory()) {
                if (!EXCLUDED_DIRECTORIES.has(lowerName))
            ...
            if (!isApplicationSourceFile(relativePath))
                continue;
            candidates.push({ absolutePath, relativePath });
            if (candidates.length > limits.maxFiles) {
                throw new DiscoveryError("file-limit-exceeded", `Discovery exceeded the ${limits.maxFiles} file safety limit`);
            }
        }
    }
    await walk(root);
    ...
    for (const candidate of candidates) {
        ...
        if (!fileStat.isFile() || fileStat.isSymbolicLink()) {
            throw new DiscoveryError("file-changed-during-scan", `A source changed type while it was being scanned: ${candidate.relativePath}`);
        }
        const canonicalFile = await realpath(candidate.absolutePath);
        if (!isWithin(root, canonicalFile)) {
            throw new DiscoveryError("path-escape-rejected", `A source resolved outside the repository: ${candidate.relativePath}`);
        }
        if (fileStat.size > limits.maxFileBytes) {
            throw new DiscoveryError("file-byte-limit-exceeded", `Source exceeds the ${limits.maxFileBytes} byte per-file safety limit: ${candidate.relativePath}`);
        }
        scannedBytes += fileStat.size;
        if (scannedBytes > limits.maxTotalBytes) {
            throw new DiscoveryError("total-byte-limit-exceeded", `Discovery exceeded the ${limits.maxTotalBytes} total byte safety limit`);
        }
