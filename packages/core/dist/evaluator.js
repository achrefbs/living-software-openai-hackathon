import { parseDetectorProgress, } from "@living-software/contracts";
import { sha256 } from "./canonical.js";
import { detectBacktrackingOpportunityWithEvidence, detectRepeatedSequenceOpportunityWithEvidence, detectTechnicalFrictionOpportunitiesWithEvidence, selectOpportunityDetection, } from "./detectors.js";
import { findBacktrackingRevisitIndexes, projectWorkflowCases, projectWorkflowJourneySteps, } from "./workflows.js";
const defaultBacktrackingConfig = {
...
const defaultRepeatedSequenceConfig = {
...
function resolvedBacktrackingConfig(overrides) {
...
function resolvedTechnicalConfig(overrides) {
    const config = { ...defaultTechnicalFrictionConfig, ...overrides };
    assertPositiveSafeInteger(config.minimumAffectedCases, "minimumAffectedCases");
    return config;
}
function resolvedRepeatedSequenceConfig(overrides) {
    const config = { ...defaultRepeatedSequenceConfig, ...overrides };
    assertPositiveSafeInteger(config.minimumAffectedCases, "minimumAffectedCases");
    assertPositiveSafeInteger(config.minimumIndependentSessions, "minimumIndependentSessions");
    assertPositiveSafeInteger(config.minimumSequenceLength, "minimumSequenceLength");
    assertPositiveSafeInteger(config.minimumOccurrencesPerCase, "minimumOccurrencesPerCase");
    ...
    if (config.minimumSequenceLength > config.maximumSequenceLength) {
        throw new TypeError("minimumSequenceLength must not exceed maximumSequenceLength");
    }
    return config;
}
...
function familyDetection(detections, detectorId) {
    return detections.find((candidate) => candidate.opportunity.detector.id === detectorId) ?? null;
}
function validateFamilyEvaluation(evaluation) {
...
function evaluateTechnicalFamily(events, config, family, detections) {
    const cases = projectWorkflowCases(events);
    const acceptedSignals = new Set(family.signals);
    const affected = cases.map((workflowCase) => workflowCase.events.filter((event) => {
    ...
    const occurrenceCount = affected.reduce((total, signalEvents) => total + signalEvents.length, 0);
...
function evaluateBacktrackingFamily(events, config, detection) {
    const cases = projectWorkflowCases(events);
    const affected = cases.map((workflowCase) => {
        ...
        const corroborated = workflowCase.events.some((event) => technicalSignal(event) !== undefined ||
            event.status === "failed" ||
            event.status === "abandoned");
        return { revisits, corroborated };
    }).filter((candidate) => candidate.revisits.length >= config.minimumRevisitsPerCase &&
        candidate.corroborated);
    const occurrenceCount = affected.reduce((total, candidate) => total + candidate.revisits.length, 0);
    const progress = Object.freeze(parseDetectorProgress({
        schemaVersion: "living.detector-progress/v1",
...
function opportunityMetric(detection, name) {
    return detection?.opportunity.signal.metrics.find((candidate) => candidate.name === name)?.observed ?? 0;
}
function evaluateRepeatedSequenceFamily(events, manifestHash, evidenceUri, config, detection) {
 ...
 */
export function evaluateOpportunityDetectors({ events, manifestHash, evidenceUri, backtrackingConfig: backtrackingOverrides, repeatedSequenceConfig: repeatedSequenceOverrides, technicalFrictionConfig: technicalOverrides, }) {
    const evaluatedEvents = [...events];
    const backtrackingConfig = resolvedBacktrackingConfig(backtrackingOverrides);
    const repeatedSequenceConfig = resolvedRepeatedSequenceConfig(repeatedSequenceOverrides);
    ...
    const repeatedSequenceDetection = detectRepeatedSequenceOpportunityWithEvidence({
        ...shared,
        config: repeatedSequenceConfig,
    });
    const families = Object.freeze([
        ...technicalFamilies.map((family) => evaluateTechnicalFamily(evaluatedEvents, technicalConfig, family, technicalDetections)),
        evaluateRepeatedSequenceFamily(evaluatedEvents, manifestHash, evidenceUri, repeatedSequenceConfig, repeatedSequenceDetection),
        evaluateBacktrackingFamily(evaluatedEvents, backtrackingConfig, backtrackingDetection),
    ]);
    const progress = Object.freeze(families.map((family) => family.progress));
    const detections = Object.freeze(families.flatMap((family) => family.detection === null ? [] : [family.detection]));
