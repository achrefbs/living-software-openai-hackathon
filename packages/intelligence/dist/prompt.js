...
export function buildResponsesRequest(opportunity, context, maxOutputTokens = 2_400) {
    ...
    const allowedMetricNames = opportunity.signal.metrics.map((metric) => metric.name);
    return {
        model: "gpt-5.6",
        store: false,
        ...
        input: [
            ...
            {
                role: "user",
                content: "Discover and draft exactly one governed EvolutionBrief from this evidence JSON. " +
