...
import { gpt56EvolutionBriefSchema, parseMetricReport, parseOpportunity, parseProductManifest, parseWorkflowEvent, } from "@living-software/contracts";
...
import { modelEvolutionBriefSchema, modelSourcePatchSchema, } from "./schema.js";
...
function extractCliUsage(value) {
...
function extractApiUsage(value) {
    const usage = asRecord(value);
    if (usage === undefined)
        return null;
...
function extractResponseEnvelope(body, transportKind) {
    ...
    if (transportKind === "codex-cli") {
        if (response.type !== "codex-cli-result" ||
    ...
    for (const itemValue of output) {
        ...
        if (item === undefined || !Array.isArray(item.content))
            continue;
        for (const contentValue of item.content) {
            ...
            if (content?.type === "output_text" && typeof content.text === "string") {
                if (actualModel === null) {
                    throw new IntelligenceResponseError("OpenAI response did not report its actual model", "malformed_response");
                }
                return {
                    text: content.text,
                    responseId,
...
function expectedDataOrigin(events) {
...
function verifyManifestHash(manifest) {
    ...
    if (sha256(semanticContent) !== contentHash) {
        throw new Error("Product manifest contentHash does not match its canonical content");
    }
}
function validateEvidence(opportunity, manifest, candidates) {
...
function validateMetricReportBinding(report, opportunity, manifest, events) {
    if (report === undefined) {
    ...
    const identityMatches = report.appId === manifest.appId &&
    ...
    const metricsMatch = metrics.length === report.values.length && report.values.every((metric, index) => {
...
function validateReferenceIntegrity(brief, opportunity, manifest, context, sampleAliasEntries) {
    const issues = [];
    if (brief.appId !== opportunity.appId || brief.appId !== manifest.appId)
        issues.push("appId");
    if (brief.opportunityId !== opportunity.opportunityId)
        issues.push("opportunityId");
    if (brief.manifestHash !== opportunity.manifestHash || brief.manifestHash !== manifest.contentHash)
        issues.push("manifestHash");
    if (brief.evidenceCitations.eventSetHash !== opportunity.evidence.eventSetHash)
        issues.push("eventSetHash");
    const eventIdByAlias = new Map(sampleAliasEntries.map((entry) => [entry.alias, entry.eventId]));
    if (brief.evidenceCitations.sampleEvidenceAliases.some((alias) => !eventIdByAlias.has(alias)))
        issues.push("sampleEvidenceAliases");
    const allowedMetrics = new Map(opportunity.signal.metrics.map((metric) => [metric.name, metric.observed]));
    if (brief.evidenceCitations.metrics.some((metric) => allowedMetrics.get(metric.name) !== metric.observed))
        issues.push("metrics");
    const allowedNodes = new Set(context.relevantProductNodeIds);
    if (brief.proposedChange.affectedProductNodeIds.some((id) => !allowedNodes.has(id)))
        issues.push("affectedProductNodeIds");
    if (brief.evidenceScope.origin !== context.evidenceScope.origin ||
    ...
    if (issues.length > 0) {
        throw new IntelligenceResponseError(`GPT-5.6 brief failed schema/reference integrity checks: ${issues.join(", ")}`, "invalid_brief");
    }
    ...
    if (!parsed.success) {
        throw new IntelligenceResponseError("GPT-5.6 brief failed canonical contract validation", "invalid_brief");
    }
    return parsed.data;
}
...
function validSourcePath(candidate) {
...
function validateSourceCandidates(candidates) {
    if (candidates.length < 1 ||
        candidates.length > SOURCE_CONTEXT_LIMITS.candidates) {
        throw new TypeError(`Source patch context requires between 1 and ${SOURCE_CONTEXT_LIMITS.candidates} candidates`);
    }
    const paths = new Set();
    let totalBytes = 0;
    const validated = candidates.map((candidate) => {
        const bytes = Buffer.byteLength(candidate.content, "utf8");
        if (!validSourcePath(candidate.path) ||
            ...
            throw new TypeError("Source candidates must be unique, bounded, hash-exact repository-relative UTF-8 files");
        }
...
function validatePatchReferenceIntegrity(proposal, input, candidates) {
    const issues = [];
    if (proposal.appId !== input.brief.appId)
        issues.push("appId");
    ...
    if (proposal.briefId !== input.brief.briefId)
        issues.push("briefId");
    const target = candidates.find((candidate) => candidate.path === proposal.target.path);
    if (target === undefined ||
    ...
    if (target !== undefined) {
        ...
        for (const edit of proposal.edits) {
            ...
            if (edit.replacement !== edit.anchor)
                changed = true;
        }
        ...
        if (!changed)
            issues.push("noChange");
    }
    if (issues.length > 0) {
        throw new IntelligenceResponseError(`GPT-5.6 source patch failed schema/reference integrity checks: ${[...new Set(issues)].join(", ")}`, "invalid_patch");
    }
    return proposal;
}
...
function baseProvenance(envelope, transportKind) {
    return {
        provider: "openai",
...
export function createIntelligenceClient(transport = createFetchTransport(), options = {}) {
    ...
    if (!Number.isInteger(maxPatchOutputTokens) ||
    ...
    return {
        async draftEvolutionBrief(input) {
            ...
            if (opportunity.appId !== manifest.appId)
                throw new Error("Opportunity and product manifest appId must match");
        ...
        async draftSourcePatch(input) {
            ...
            return {
                proposal: validatePatchReferenceIntegrity(parsed.data, { brief, candidates }, candidates),
                provenance: {
                    ...baseProvenance(envelope, transportKind),
