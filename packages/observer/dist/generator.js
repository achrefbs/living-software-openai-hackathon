...
function string(input, path, options) {
...
function integer(input, path, minimum, maximum) {
...
function binding(input, path) {
    ...
    const surfaceId = value.surfaceId === undefined
        ? undefined
...
function routePattern(input, path) {
...
function locator(input, path) {
    ...
    if (strategy === "structure") {
        ...
        if (!STRUCTURAL_TAGS.has(tag))
            throw new TypeError(`${path}.tag is invalid`);
        let ancestorTags;
        ...
        const ordinalWithinParent = value.ordinalWithinParent === undefined
            ? undefined
...
function normalizeRuntimeMap(input) {
    ...
    exactKeys(applicationInput, ["appId", "environment", "releaseRevision", "manifestHash", "synthetic"], "runtimeMap.application");
    const environment = string(applicationInput.environment, "runtimeMap.application.environment", { max: 16 });
    ...
    const application = Object.freeze({
        ...
        releaseRevision: string(applicationInput.releaseRevision, "runtimeMap.application.releaseRevision", { max: 160 }),
        manifestHash: string(applicationInput.manifestHash, "runtimeMap.application.manifestHash", { max: 71, pattern: MANIFEST_HASH }),
        synthetic: boolean(applicationInput.synthetic, "runtimeMap.application.synthetic"),
    });
    ...
    const collector = Object.freeze({ endpoint: "/api/living/events" });
    ...
    const targets = root.targets.map((candidate, targetIndex) => {
        ...
        if (targetTokens.has(token))
            throw new TypeError(`${path}.token must be unique`);
        targetTokens.add(token);
        if (!Array.isArray(targetInput.locators) ||
        ...
        exactKeys(eventsInput, ["click", "change", "submit", "deadClick", "rageClick", "correction"], `${path}.events`);
    ...
    const routes = root.routes.map((candidate, routeIndex) => {
        ...
        if (routePatterns.has(pattern))
            throw new TypeError(`${path}.pattern must be unique`);
    ...
    exactKeys(systemInput, ["sessionEnd", "runtimeError", "lcp", "inp", "cls"], "runtimeMap.systemEvents");
    ...
    exactKeys(signalsInput, ["deadClickDelayMs", "rageClickWindowMs", "rageClickCount", "correctionWindowMs"], "runtimeMap.signals");
    const signals = Object.freeze({
        deadClickDelayMs: integer(signalsInput.deadClickDelayMs, "runtimeMap.signals.deadClickDelayMs", 250, 5_000),
        rageClickWindowMs: integer(signalsInput.rageClickWindowMs, "runtimeMap.signals.rageClickWindowMs", 250, 10_000),
        rageClickCount: integer(signalsInput.rageClickCount, "runtimeMap.signals.rageClickCount", 2, 10),
        correctionWindowMs: integer(signalsInput.correctionWindowMs, "runtimeMap.signals.correctionWindowMs", 250, 30_000),
    });
    const limitsInput = record(root.limits, "runtimeMap.limits");
    exactKeys(limitsInput, [
        "maxBatchSize",
        "maxQueueSize",
        "maxEventBytes",
        "maxPayloadBytes",
    ...
    const limits = Object.freeze({
        ...
        maxEventsPerMinute: integer(limitsInput.maxEventsPerMinute, "runtimeMap.limits.maxEventsPerMinute", 1, 10_000),
    ...
    for (const declaration of allBindings) {
        ...
        if (previous !== undefined && previous !== signature) {
            throw new TypeError(`Event '${declaration.eventName}' cannot point to multiple product nodes or kinds`);
        }
...
`;
export function generateNextObserverFiles(input) {
...
`;
    return Object.freeze({
        instrumentationClient: Object.freeze({
            relativePath: "src/instrumentation-client.ts",
            content: INSTRUMENTATION_CLIENT_SOURCE,
        }),
        browserModule: Object.freeze({
            relativePath: "src/living-observer.generated.ts",
