...
| Intelligence | Explicit Codex CLI (`gpt-5.6-terra`) and Responses API (`gpt-5.6`) transports, exact neutral evidence/hash checks, privacy-minimized context, opaque evidence aliases, strict structured output, provider-specific isolation/bounds, truthful model provenance, and local reference validation | Automatic-host evidence ingestion, semantic truth guarantees, approval, code generation, host tools, or activation; a successful live proof run is still pending |
